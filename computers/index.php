...
// بناء الاستعلام مع البحث
 $search_condition = $search ? "WHERE asset_tag LIKE '%$search%' OR make LIKE '%$search%' OR model LIKE '%$search%' OR location LIKE '%$search%'" : "";
 $sql = "SELECT id, asset_tag, type, make, model, location, status FROM computers $search_condition ORDER BY id DESC LIMIT ?, ?";
...
// استعلام للحصول على العدد الإجمالي للسجلات
 $total_sql = "SELECT COUNT(*) as total FROM computers $search_condition";
...
<head>
    <meta charset="UTF-8">
    <title>إدارة أجهزة الحاسب</title>
    ...
    <main class="container">
        <div class="form-container">
            ...
            <form class="search-form" method="GET" action="index.php">
                <input type="text" name="search" placeholder="بحث عن جهاز..." value="<?php echo htmlspecialchars($search); ?>">
            ...
            <table class="employees-table">
                ...
                <tbody>
                    <?php if ($result->num_rows > 0): ?>
                        <?php while($row = $result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    if (strtolower($row['status']) == 'قيد الاستخدام' || strtolower($row['status']) == 'يعمل') {
                                        $status_class = 'status-working';
                                    } elseif (strtolower($row['status']) == 'تحت الصيانة') {
                                        $status_class = 'status-maintenance';
                                    } elseif (strtolower($row['status']) == 'معطل' || strtolower($row['status']) == 'معطلة') {
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan='6'>لا توجد أجهزة حاسب مسجلة</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
