...
// بناء الاستعلام مع البحث
 ...
 $sql = "SELECT id, asset_tag, type, make, model, location, status, ip_address FROM printers $search_condition ORDER BY id DESC LIMIT ?, ?";
...
// استعلام للحصول على العدد الإجمالي للسجلات
 $total_sql = "SELECT COUNT(*) as total FROM printers $search_condition";
    ...
    <main class="container">
        <div class="form-container">
            <div class="actions-bar">
                <h2>قائمة الطابعات</h2>
                <a href="add.php" class="btn" style="background-color: var(--success-color);">
                    <i class="fas fa-plus-circle"></i> إضافة طابعة جديدة
                </a>
            </div>
            
            <form class="search-form" method="GET" action="index.php">
                <input type="text" name="search" placeholder="بحث عن طابعة..." value="<?php echo htmlspecialchars($search); ?>">
            ...
            <table class="employees-table">
                ...
                <tbody>
                    <?php if ($result->num_rows > 0): ?>
                        <?php while($row = $result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    } elseif (strtolower($row['status']) == 'معطل' || strtolower($row['status']) == 'تحتاج حبر/حبر نافذ') {
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan='7'>لا توجد طابعات مسجلة</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
